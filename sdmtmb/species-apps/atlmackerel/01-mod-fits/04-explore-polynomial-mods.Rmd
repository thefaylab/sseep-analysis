...
## Objective 
Initial model fitting shows: 
* a smoother on depth has a better fit that a quadratic polynomial fit on depth, 
* including area as a covariate resulted in a worse model fit, and 
* removing outliers of biomass has no effect on model parsimony. 

Script will: 
* explore a third and fourth order polynomial relationship with depth to assess whether it results in a better model fit than the spline, 
* compare the model fits using a tweedie probability distribution and a delta poisson gamma distribution, and
* assess whether removing observations at depths greater than 200m improves model fit and marginal effects. 
...
tm_polymods <- list(m1_spring, m2_spring, m3_spring, m4_spring)

# table of AIC values 
tm_polymods.diag <- data.frame(models = c("m1", "m2", "m3", "m4"),
                           spatial = "off",
                           spatiotemporal = "off", 
                           time = "EST_YEAR",
                           season = "SPRING",
...
tm.mods.form <- map(tm_polymods, ~pluck(., "formula") |> as.character()) |> as.data.frame() |> t()
tm.mods.conv <- map(tm_polymods, ~pluck(., "sd_report", "pdHess")) |> as.data.frame() |> t()
tm.mods.aic <- map(tm_polymods, ~AIC(.)) |> as.data.frame() |>  t()

tm.mods.diag <- tm_polymods.diag |> 
  mutate(formula = tm.mods.form, 
         converged = tm.mods.conv, 
         AIC = tm.mods.aic)

kable(tm.mods.diag, align = "lcccc", caption = "Atlantic mackerel spring model with varying depth relationships", format.args = list(big.mark = ","), booktabs = TRUE) |>
...
kable(tweedie.diag, align = "lcccc", caption = "Atlantic mackerel spring model with varying depth relationships", format.args = list(big.mark = ","), booktabs = TRUE) |>
     kable_styling(full_width = F, fixed_thead = T, font_size = 14)
...
am_spring1 <- am_spring |> 
  filter(AVGDEPTH <= 200)
saveRDS(am_spring1, here("sdmtmb", "atlmackerel", "data", "atlmackerel_spring_rm200.rds"))

spring_mesh1 <- make_mesh(am_spring1, xy_cols = c("X", "Y"), cutoff = 20)
saveRDS(spring_mesh1, here("sdmtmb", "atlmackerel", "data", "spring_mesh_rm200.rds"))
```
...
kable(tm_rm200.diag, align = "lcccc", caption = "Atlantic mackerel spring model with outliers removed", format.args = list(big.mark = ","), booktabs = TRUE) |>
     kable_styling(full_width = F, fixed_thead = T, font_size = 14)
