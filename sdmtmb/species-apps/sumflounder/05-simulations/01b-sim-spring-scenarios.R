...
resampled_years <- sample(c(2009:2021), size = 5, replace = TRUE)
...
base.sim <- sdmTMB_simulate(
  ...
  time = "EST_YEAR",
  #spatiotemporal = "iid",
  B = c(-0.06, 7.66, -27.7, 0.338, 1), # coefficient estimates 
  range = 96.2, 
...
inc.sim <- sdmTMB_simulate(
  ...
  #spatial = "on", 
  #spatiotemporal = "iid",
  B = c(-0.06, 7.66, -27.7, (0.338+log(2)), 1), # coefficient estimates; increase scenario = double of estimated wind coefficient (0.338); additive -> log(pred.effect*2) = log(pred.effect) + log(2)
  range = 96.2, 
  #rho = 0.158, # AR1 correlation
  sigma_O = 0,#1.15,
  sigma_E = 0.844,
...
dec.sim <- sdmTMB_simulate(
  ...
  time = "EST_YEAR",
  #spatiotemporal = "iid",
  B = c(-0.06, 7.66, -27.7, (0.338-log(2)), 1), # coefficient estimates; reduction = half of estimated wind coefficient (0.338); subtractive -> log(pred.effect/2) = log(pred.effect) - log(2)
  range = 96.2, 
  #rho = 0.158, # AR1 correlation
  sigma_O = 0,#1.15,
  sigma_E = 0.844,
