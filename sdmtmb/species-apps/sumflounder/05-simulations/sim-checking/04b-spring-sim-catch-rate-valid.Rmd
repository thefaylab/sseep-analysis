...
```{r sim increase}
# b <- c(0, spring_coefs$estimate[1:2], spring_coefs$estimate[10],
#        (spring_coefs$estimate[15]+log(2)), 1)
...
head(spring.dec)
# saveRDS(spring.dec, here(sim.check, "spring_dec-100sims.rds"))
```
...
spring.base.df <- map(spring.base, ~janitor::clean_names(.)) |> 
  ...
  mutate(scenario = "Baseline", 
         season = "Spring") |> 
  left_join(sf_spring_info, by = join_by(x == X, y == Y, est_year == EST_YEAR, as_factor_area_wind == AREA_CODE)) |> 
...
bind_rows(spring.base.df, spring.inc.df, spring.dec.df) |> 
  #filter(observed > 0) |> 
    ggplot() + 
    geom_histogram(aes(x = observed, fill = as.factor(as_factor_area_wind)), binwidth = 2) + 
  scale_fill_manual(values = pal) +
  facet_grid(cols = vars(scenario), rows = vars(str_to_title(AREA))) + 
  labs(x = "Catch rate (kg/tow)", y = "Frequency of catch rate occurrence", subtitle = str_c("Distribution of simulated values of spring summer flounder biomass for each productivity scenario observed in\nstratum", strat, "in", year, sep = " "), fill = "Area of catch\nrate occurrence") + 
  theme(legend.position = "none") + xlim(NA, 25)
  
# ggsave("spring_100sim_catchrate-dist2.png", device = "png", last_plot(), here(sim.check.plots), width = 9, height = 6)
```
