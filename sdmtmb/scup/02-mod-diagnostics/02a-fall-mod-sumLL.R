...
# read in models
fall.cv <- str_c("m",c(1:8), "-fall-cv.rds") |>
  #append(str_c("m",c(8, 11, 15:16), "-fall-cv2.rds")) |>
  map(~list(.)) |>
  map(~readRDS(here("sdmtmb",  "scup", "data", "cross-valid", .)))

mod.names <- str_c("m",c(1:8), ".cv") #|> 
...
cvs <- sum_logliks |> 
  ...
  bind_cols(convergence) |> 
  arrange(desc(sum_logliks))
  
  
# save the data
saveRDS(cvs, file = here("sdmtmb", "scup", "data", "fall-cvs.rds"))


kable(cvs, align = "lcccc", caption = "Fall Cross Validations", format.args = list(big.mark = ","), booktabs = TRUE) %>%
