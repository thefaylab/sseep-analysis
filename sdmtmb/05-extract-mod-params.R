...
x_mat <- poly(mod$data$AVGDEPTH, 4)
...
rp.tbl <- ran_pars |> column_to_rownames(var = "Parameter") |>
  gt(rownames_to_stub = TRUE) |> 
  tab_stubhead(label = "Parameter") |>
  fmt_number() |>
  tab_header(title = md("Estimates for random parameters from the", str_c(str_to_title(season), str_to_title(species_name), "model", sep = " "))) |>
  tab_options(table.width = pct(40), 
              column_labels.font.weight = "bold")
