...
fall_simdat2x <- bind_cols(x2, info) |> 
  rename(EXPCATCHWT = observed) |>
  mutate(SVSPP = 103)
saveRDS(fall_simdat2x, here("sdmtmb", "data", "simdat_fall2x.rds"))

FallMu_ww2x <- strata.mean(fall_simdat2x) |>
  mutate(SEASON = "FALL",
...
FallMu_wow2x <- strata.mean(fall_simdat2x |> filter(AREA == "OUTSIDE")) |>
  mutate(SEASON = "FALL",
         sdlog = sqrt(log(1+(sqrt(stratvar)/stratmu)^2)), #logistic standard deviation
         lower = qlnorm(0.025, log(stratmu), sdlog), # lower quantile of the logistic normal distribution
         upper = qlnorm(0.975, log(stratmu), sdlog)) |> # upper quantile of the logistic normal distribution
...
fall_sim_mu2x <- bind_rows(fall_stratmeans, FallMu_wow2x, FallMu_ww2x)
saveRDS(fall_sim_mu2x, here("sdmtmb", "data", "fall_sim_mu2x.rds"))
...
ggplot() + 
  ...
  geom_pointrange(data = fall_sim_mu2x, aes(x = as.factor(EST_YEAR), y = stratmu, color = METHOD, shape = TYPE, ymin=lower, ymax = upper), position =  position_dodge2(width=0.4)) +
...
ggplot() +
  geom_point(data = fall_simdat2x, mapping = aes(X*1000, Y*1000, color = EXPCATCHWT)) +
...
simdat_spr2x <- bind_cols(y2, spr_info) |> 
  rename(EXPCATCHWT = observed) |>
  mutate(SVSPP = 103)

saveRDS(simdat_spr2x, here("sdmtmb", "data", "simdat_spr2x.rds"))

SprMu_ww2x <- strata.mean(simdat_spr2x) |>
...
SprMu_wow2x <- strata.mean(simdat_spr2x |> filter(AREA == "OUTSIDE")) |>
  mutate(SEASON = "SPRING",
         sdlog = sqrt(log(1+(sqrt(stratvar)/stratmu)^2)), #logistic standard deviation
         lower = qlnorm(0.025, log(stratmu), sdlog), # lower quantile of the logistic normal distribution
         upper = qlnorm(0.975, log(stratmu), sdlog)) |> # upper quantile of the logistic normal distribution
...
spr_sim_mu2x <- bind_rows(spr_stratmeans, SprMu_wow2x, SprMu_ww2x)
saveRDS(spr_sim_mu2x, here("sdmtmb", "data", "spr_sim_mu2x.rds"))

ggplot() + 
  #geom_pointrange(data = stratmeans |> , aes(x = as.factor(EST_YEAR), y = stratmu, color = TYPE, shape = TYPE, ymin=lower, ymax = upper), position =  position_dodge2(width=0.4)) + #geom_point() +
  geom_pointrange(data = spr_sim_mu2x, aes(x = as.factor(EST_YEAR), y = stratmu, color = METHOD, shape = TYPE, ymin=lower, ymax = upper), position =  position_dodge2(width=0.4)) +
  facet_wrap(vars(SEASON), scales = "free_y") +
  #facet_grid(rows = vars(GEO_AREA), cols = vars(SEASON), scales = "free_y") +
...
ggplot() +
  geom_point(data = simdat_spr2x, mapping = aes(X*1000, Y*1000, color = EXPCATCHWT)) +
  geom_sf(data = east_coast) +
  scale_color_viridis_c(trans = "sqrt", option = "H") +
  facet_wrap(~EST_YEAR) + 
  labs(x = "Longitude", y = "Latitude")
