...
sim.dat <- here("sdmtmb", species, "data", "simulations")#, "min-sims")
...
# create a list item containing the historical survey locations replicated by the number of nsims 
...
# save the data
saveRDS(sim_grid, here(sim.dat, str_c(season, species, length(nsims), "surv_grid.rds", sep = "_")))
...
grid_data <- map2(sim_grid, grid_meshs, ~append(.x, .y)) |> 
  map2(coefs, ~append(.x, .y))
