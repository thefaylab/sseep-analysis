...
### DATA #### 
# load best fitting m12 model created in 01-mod-fits.R and resaved as fall_mod after diagnostics 
...
depth_reg <- visreg(fall_mod, xvar = "AVGDEPTH", plot = FALSE)
...
ggsave("fall-depth-cond-effects.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-cond-effects"), width = 6, height = 4, plot = last_plot())
saveRDS(depth_reg, here("sdmtmb", "sumflounder", "data", "depth_cond_effects_dat.rds"))

#### IN RESPONSE SPACE ####
depth_resp_plot <- visreg(fall_mod, xvar = "AVGDEPTH", scale = "response", gg = TRUE)
saveRDS(depth_resp_plot, here("sdmtmb", "sumflounder", "data", "depth-resp_cond_effects_dat.rds"))
...
ggsave("fall-depth-resp-cond-effects.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-cond-effects"), width = 8, height = 4, plot = last_plot())


#### & YEAR ####
depth_yr_reg <- visreg(fall_mod, xvar = "AVGDEPTH", by = "EST_YEAR", plot = FALSE)
...
ggsave("fall_dep_yr-cond-effects.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-cond-effects"), width = 8, height = 6, plot = last_plot())
saveRDS(depth_yr_reg, here("sdmtmb", "sumflounder", "data", "fall-depth-year_condeffects-dat.rds"))

##### IN RESPONSE SPACE ####
depth_yr_resp <- visreg(fall_mod, xvar = "AVGDEPTH", by = "EST_YEAR", scale = "response", gg = TRUE)
...
ggsave("dep_yr-response-cond-effects.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-cond-effects"), width = 8, height = 4, plot = last_plot())
saveRDS(depth_yr_resp, here("sdmtmb", "sumflounder", "data", "fall-depth-year-response_condeffects-dat.rds"))

##### 2D PLOT ####
visreg2d(fall_mod, xvar = "EST_YEAR", yvar = "AVGDEPTH", plot.type = "gg")
ggsave("dep_yr-cond-effects2.png", device = "png", here("sdmtmb", "sumflounder", "plots", "cond-effects"), width = 6, height = 4, plot = last_plot())
# saveRDS(depth_yr_2dplot, here("sdmtmb", "sumflounder", "data", "depth-year_2Deffects-plot.rds"))

#### & AREA ####
depth_area_reg <- visreg(fall_mod, xvar = "AVGDEPTH", by = "AREA", plot = FALSE)
...
##### IN RESPONSE SPACE ####
depth_area_resp <- visreg(fall_mod, xvar = "AVGDEPTH", by = "AREA", scale = "response", gg = TRUE)
...
# 2D plot
depth_area2d <- visreg2d(fall_mod, xvar = "AREA", yvar = "AVGDEPTH", plot.type = "gg")
...
### YEAR ####
#year_reg <- visreg(fall_mod, xvar = "EST_YEAR", plot = FALSE)
...
year_eff <- visreg(fall_mod, xvar = "EST_YEAR", gg = TRUE)
...
ggsave("fall-year-cond-effects.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-cond-effects"), width = 8, height = 4, plot = last_plot())
saveRDS(year_eff, here("sdmtmb", "sumflounder", "data", "fall-year_condeffects-dat.rds"))

#### & AREA ####
year_area <- visreg(fall_mod, xvar = "EST_YEAR", by = "AREA", gg = TRUE)
...
### AREA ####
area_plot <- visreg(fall_mod, xvar = "AREA", gg = TRUE)
...
### DEPTH ####
g <- ggpredict(fall_mod, "AVGDEPTH [all]") |> plot()
plot(g) + labs(x = "Average Depth", y = "Biomass (kg)", title="Predicted values of fall summer flounder biomass")
saveRDS(g, here("sdmtmb", "sumflounder", "data", "fall-depth_marg-eff-dat.rds"))
ggsave("sf-fall_depth_marg-effects.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-marg-effects"), width = 6, height = 4, plot = last_plot())

### YEAR ####
g2 <- ggpredict(fall_mod, "EST_YEAR [all]") |> plot()
saveRDS(g2, here("sdmtmb", "sumflounder", "data", "fall-year_marg-eff-dat.rds"))
plot(g2) + labs(x = "Year", y = "Biomass (kg)", title="Predicted values of fall summer flounder biomass")
ggsave("sf-fall_year_marg-effects.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-marg-effects"), width = 8, height = 4, plot = last_plot())

### AREA ####
g3 <- ggpredict(fall_mod, "AREA [all]") |> plot()
saveRDS(g3, here("sdmtmb", "sumflounder", "data", "fall-area_marg-eff-dat.rds"))

plot(g3) + labs(x = "Area", y = "Biomass (kg)", title="Predicted values of fall summer flounder biomass")
ggsave("sf-fall_area_marg-effects.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-marg-effects"), width = 6, height = 4, plot = last_plot())

### DEPTH & YEAR ####
g4 <- ggpredict(fall_mod, terms = c("AVGDEPTH [all]", "EST_YEAR [all]")) #|> plot()
saveRDS(g4, here("sdmtmb", "sumflounder", "data", "fall-depth-yr_marg-eff-dat.rds"))
...
ggsave("sf-fall_dep-yr_marg-effects1.png", device = "png", here("sdmtmb", "sumflounder", "plots", "marg-effects"), width = 7, height = 5, plot = last_plot())

ggplot(g4, aes(x, predicted, colour = group)) +
  geom_line() + 
  labs(x = "Average Depth", y = "Biomass (kg)", color = "Year", subtitle = "Predicted values of fall summer flounder biomass") + xlim(0, NA) +
  facet_wrap(~group)
ggsave("sf-fall_dep-yr_marg-effects2.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-marg-effects"), width = 6, height = 4, plot = last_plot())

### DEPTH & AREA ####
g5 <- ggpredict(fall_mod, terms = c("AVGDEPTH [all]", "AREA [all]"))
saveRDS(g5, here("sdmtmb", "sumflounder", "data", "fall-dep-area_marg-eff-dat.rds"))

plot(g5, facet = TRUE) + 
  labs(x = "Average Depth", y = "Biomass (kg)", title = "Predicted values of fall summer flounder biomass")
ggsave("sf-fall_depth-area_marg-effects.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-marg-effects"), width = 6, height = 4, plot = last_plot())

### YEAR & AREA ####
g6 <- ggpredict(fall_mod, terms = c("EST_YEAR [all]", "AREA [all]"))
saveRDS(g6, here("sdmtmb", "sumflounder", "data", "fall-area-yr_marg-eff-dat.rds"))

plot(g6, facet = TRUE) + 
  labs(x = "Year", y = "Biomass (kg)", title = "Predicted values of fall summer flounder biomass")
ggsave("sf-fall_year-area_marg-effects.png", device = "png", here("sdmtmb", "sumflounder", "plots", "fall-marg-effects"), width = 10, height = 5, plot = last_plot())
