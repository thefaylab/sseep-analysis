...
bind_rows(spring.base.df, spring.inc.df, spring.dec.df) |> 
  #filter(observed > 0) |> 
    ggplot() + 
    geom_histogram(aes(x = observed, fill = as.factor(as_factor_area_wind)), position = "dodge") + 
...
# ggsave("spring_100sim_catchrate-dist.png", device = "png", last_plot(), here(sim.check.plots), width = 9, height = 6)
```
