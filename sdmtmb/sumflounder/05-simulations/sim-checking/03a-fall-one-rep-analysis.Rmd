...
Compare to base wind tows chunk above to ensure changes in `eta` and `observed`
```{r inc wind tows}
...
```{r}
abs_diff <- all_stratmu |> 
  group_by(SCENARIO) |> 
  summarise(diff_mu = abs(diff(stratmu)), 
            avg_mu = mean(stratmu)) |>
  mutate(percent_diff = round((diff_mu / avg_mu)*100, 0))

kable(abs_diff[,c(1,4)], col.names = c("Treatment", "Absolute percent difference")) |> kable_styling()
...
ggplot(all_stratmu) +
  ...
  labs(x = "Wind-driven productivity scenario", y = "Stratified Mean (kg/tow)", subtitle = "Changes in the 2016 abundance index for summer flounder in response to changes in productivity inside wind energy areas") +
  ylim(0,NA) +
  facet_wrap(~season)
  theme_bw() + 
