...
fall.inc <- map2(1:500, seed.num, ~sdmTMB_simulate(
    formula = ~1 +poly(AVGDEPTH, 2, coefs = fall_depth_coefs) +
      EST_YEAR +
      as.factor(AREA) +
      fix_omegas,
...
head(fall.inc)
# saveRDS(fall.inc, here(sim.check, "fall_inc-500sim.rds"))
```
...
fall.dec <- map2(1:500, seed.num, ~sdmTMB_simulate(
    formula = ~1+ poly(AVGDEPTH, 2, coefs = fall_depth_coefs) +
      EST_YEAR +
      as.factor(AREA) +
      fix_omegas,
...
head(fall.dec)
# saveRDS(fall.dec, here(sim.check, "fall_dec-500sim.rds"))
```
...
filtered.base.df <- map(fall.base, ~janitor::clean_names(.)) |> 
...
```{r}
# create one large dataframe and add a sim column 
filtered.inc.df <- map(fall.inc, ~janitor::clean_names(.)) |> 
  map2_dfr(1:500, ~mutate(.x, sim = .y))
...
```{r}
# create one large dataframe and add a sim column 
filtered.dec.df <- map(fall.dec, ~janitor::clean_names(.)) |> 
  map2_dfr(1:500, ~mutate(.x, sim = .y))
...
bind_rows(base_diff, inc_diff, dec_diff) |> 
  ...
  labs(x = "Productivity scenario", y = "Absolute percent difference between survey effort scenarios", subtitle = str_c("Distribution of differences between survey effort scenarios for each productivity scenario\nobserved in stratum", strat, "in", year, "and across 500 replicates", sep = " ")) + 
  theme_bw()

# to extract exact median values plotted in the boxplot
median(base_diff$abs_diff)
median(inc_diff$abs_diff)
median(dec_diff$abs_diff)

# ggsave("fall_500sim_diff-dist.png", device = "png", last_plot(), here(sim.check.plots), width = 8, height = 6)
```
