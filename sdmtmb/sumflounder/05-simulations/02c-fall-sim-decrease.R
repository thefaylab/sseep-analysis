...
x <- sdmTMB_simulate(
  ...
  time = "EST_YEAR",
  B = c(-0.66, -41.8, -25.4, log(0.0545)), # coefficient estimates 
  range = 74.5, 
...
decrease_dat <- bind_cols(x, fall_info) |> 
  rename(EXPCATCHWT = observed) |>
  mutate(SVSPP = 103, 
         rep = i)

decrease_sims <- bind_rows(decrease_sims, decrease_dat)

}

saveRDS(decrease_sims, here("sdmtmb", "sumflounder", "data", "simulations", "fall_decrease_sim.rds"))
...
saveRDS(decrease_stratmu, here("sdmtmb", "sumflounder", "data", "simulations", "fall_dec_sim-stratmu.rds"))
...
# save the data 
saveRDS(full_stratmu, here("sdmtmb", "sumflounder", "data", "simulations", "fall_dec_all-stratmu.rds"))
