...
## Objective 
* generate random numbers from the model fit to Atlantic mackerel with varying scales of a mean (control)
* generate random numbers from the model fit to Atlantic mackerel with varying scales of a mean plus the intended treatment effect (treatment)
...
# read in fall model 
mod <- readRDS(here(atlmackerel.dat, str_c(season, "mod.rds", sep = "_")))
...
bind_rows(base1_diff, enhanced1_diff, reduced1_diff) |>
  ggplot() + 
  geom_boxplot(aes(x = as.factor(type), y = abs_diff_mu)) + 
...
ggplot(all_treatments) +
  ...
  labs(x = "Productivity treatment", y = "Absolute percent difference\nbetween effort scenario means", color = "Number-generating mean", subtitle = paste0("Tweedie distributions generated from the"," ", season, " ", "Atlantic mackerel model parameters")) + 
  theme_bw() + 
...
ggplot(all_treatments) +
  ...
  labs(x = "Productivity treatment", y = "Absolute percent difference\nbetween effort scenario CVs", color = "Number-generating mean", subtitle = paste0("Tweedie distributions generated from the"," ", season, " ", "Atlantic mackerel model parameters")) + 
  theme_bw() + 
  theme(legend.position = "bottom")
