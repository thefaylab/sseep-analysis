...
# base_diff <- bind_cols(spring_base_ww, spring_base_wow) |># , by = c("sim", "scenario")) |>
#   janitor::clean_names() |>
#   # group_by(sim) |> #, est_year) |>
#   summarise(abs_diff_mu =( abs(mu_1 - mu_5) / mu_1)*100, 
#             abs_diff_cv =( abs(cv_3 - cv_7) / cv_3)*100) |>
...
# 
# # bind the scenarios to calculate the absolute percent difference 
# inc_diff <- bind_cols(spring_inc_ww, spring_inc_wow) |> #, by = c("sim", "scenario")) |>
#   janitor::clean_names() |>
#   # group_by(sim) |> #, est_year) |>
#   summarise(abs_diff_mu =( abs(mu_1 - mu_5) / mu_1)*100, 
#             abs_diff_cv =( abs(cv_3 - cv_7) / cv_3)*100) |>
...
# 
# # bind the scenarios to calculate the absolute percent difference 
# dec_diff <- bind_cols(spring_dec_ww, spring_dec_wow) |> #, by = c("sim", "scenario")) |>
#   janitor::clean_names() |>
#   # group_by(sim) |> #, est_year) |>
#   summarise(abs_diff_mu =( abs(mu_1 - mu_5) / mu_1)*100, 
#             abs_diff_cv =( abs(cv_3 - cv_7) / cv_3)*100) |>
...
bind_rows(spring.base.df, spring.inc.df, spring.dec.df) |>
  #filter(observed > 0) |> 
     ggplot() + 
     geom_histogram(aes(x = observed, fill = AREA), bins = 10) + 
