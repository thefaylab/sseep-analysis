---
title: "Atlantic mackerel one rep analysis"
...
spring_subset <- readRDS(here(sim.check, "subset_spring-resampled_data.rds")) 

# the year that was used to subset the data in "02a-check-spring-sim.Rmd"
year <- readRDS(here(sim.check, "resample_year.rds"))
...
```{r sim base}
# spring
b <- c(0, spring_coefs$estimate[1:4], spring_coefs$estimate[15], 0, 1)
...
```{r}
names <- c(names(spring.base[1:7]), "Intercept", "depth_1", "depth_2", "depth_3", "depth_4", "year_covar", "wind_covar",names(spring.base[15]))
...
### Enhanced
```{r sim enhanced}
b <- c(0, spring_coefs$estimate[1:4], spring_coefs$estimate[15], log(2), 1)
...
```{r}
names <- c(names(spring.inc[1:7]), "Intercept", "depth_1", "depth_2", "depth_3", "depth_4", "year_covar", "wind_covar",names(spring.inc[15]))
...
```{r}
names <- c(names(spring.dec[1:7]), "Intercept", "depth_1", "depth_2", "depth_3", "depth_4", "year_covar", "wind_covar",names(spring.dec[15]))
...
spring.base.stratmu
# saveRDS(spring.base.stratmu.ww, here(sim.check, "atlmackerel_spring_base-one-sim-stratmu-ww.rds"))
# saveRDS(spring.base.stratmu.wow, here(sim.check, "atlmackerel_spring_base-one-sim-stratmu-wow.rds"))
# saveRDS(spring.base.stratmu, here(sim.check, "atlmackerel_spring_base-one-sim-stratmu.rds"))
...
# saveRDS(spring.inc.stratmu.ww, here(sim.check, "atlmackerel_spring_inc-one-sim-stratmu-ww.rds"))
# saveRDS(spring.inc.stratmu.wow, here(sim.check, "atlmackerel_spring_inc-one-sim-stratmu-wow.rds"))
# saveRDS(spring.inc.stratmu, here(sim.check, "atlmackerel_spring_inc-one-sim-stratmu.rds"))
```
...
spring.dec.stratmu
# 
# saveRDS(spring.dec.stratmu.ww, here(sim.check, "atlmackerel_spring_dec-one-sim-stratmu-ww.rds"))
# saveRDS(spring.dec.stratmu.wow, here(sim.check, "atlmackerel_spring_dec-one-sim-stratmu-wow.rds"))
# saveRDS(spring.dec.stratmu, here(sim.check, "atlmackerel_spring_dec-one-sim-stratmu.rds"))
...
mudiff_dat
# saveRDS(mudiff_dat, here(sim.check, "atlmackerel_spring_one-sim-mudiff.rds"))
```
...
all_stratmu
# saveRDS(all_stratmu, here(sim.check, "atlmackerel_spring_one-sim-stratmu.rds"))
```
...
ggplot(all_stratmu) +
  ...
  labs(x = "Wind-driven productivity scenario", y = "Stratified Mean (kg/tow)", title = "Spring productivity scenarios for Atlantic mackerel", subtitle = "Changes in the 2019 abundance index for Atlantic mackerel in response to changes in productivity inside wind energy areas") +
  ylim(0,NA) +
  theme_bw() + 
...
# ggsave("atlmackerel_spring-sim-one-rep.png", device = "png", last_plot(), here(sim.check.plots), width = 9, height = 6)
```
