...
#### 06b - STRATIFIED CALCULATIONS: WITH WIND AREAS PRECLUDED ####
...
m6fall_sim <- readRDS(here("sdmtmb", "data", "sf-fall-with-m6sims.rds")) %>% filter(AREA == "OUTSIDE")
m6spr_sim <- readRDS(here("sdmtmb", "data", "sf-spr-with-m6sims.rds")) %>% filter(AREA == "OUTSIDE")

# sf_m6sim <- readRDS(here("sdmtmb", "data", "sumflounder-m6sims.rds")) 
...
saveRDS(FallSimMu_wow, here("sdmtmb", "data", "sf-fall_sim-wow-means.rds"))

SprSimMu_wow <- m6spr_sim %>% 
  pivot_longer(cols = c(sim1:sim1000), 
               names_to = "nsim", 
               values_to = "EXPCATCHWT") %>% 
  group_by(nsim) %>% 
  nest() %>%
  mutate(stratmu = map(data, ~strata.mean(.)))
saveRDS(SprSimMu_wow, here("sdmtmb", "data", "sf-spr_sim-wow-means.rds"))
