...
catch_lost_plot <- species_impact |> 
  ...
  ggplot() + 
  geom_col(aes(x = str_wrap(COMNAME, width = 10, whitespace_only = FALSE), y = WIND_OVERLAP_CATCH), fill = "#0B6E4F") + 
  labs(x = "Species", y = "Percentage of observations removed", title = "Percentage of survey observations removed due to spatial overlap with wind energy areas") +  
  ylim(0, 40) +
  coord_flip() + 
  theme_bw()+
...
tows_lost_plot <- species_impact |> 
  ...
  mutate(COMNAME = str_to_sentence(COMNAME)) |>
  ggplot() + 
  geom_col(aes(x = str_wrap(COMNAME, width = 10, whitespace_only = FALSE), y = WIND_OVERLAP_TOW), fill = "#0a4c8a") + 
  labs(x = "Species", y = "Percentage of tows removed", title = "Percentage of survey tows removed due to spatial overlap with wind energy areas") + 
  ylim(0, 40)+
  coord_flip() +
  theme_bw()+
  theme(title = element_text(size = 16, hjust = 0.5, margin = margin(5, 0, 10, 0, "pt")), axis.title.x = element_text(size = 18, margin = margin(10, 0, 5, 0, "pt")), axis.title.y = element_text(size = 18, margin = margin(0, 10, 0, 5, "pt")), axis.text = element_text(size = 16), axis.text.y = element_text(margin = margin(75, 10, 75, 0, "pt")))
