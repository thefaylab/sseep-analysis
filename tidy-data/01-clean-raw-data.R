...
## This script produces a clean full data set that can be used by all subsequent code/analyses.
# Script should:
# read in the bottom trawl data,
# aggregate spiny dogfish & skates
# create a towid column. (TOWID)
# write a csv & rds file containing the clean data object
...
sd_bts <- bts %>% 
          ...
          group_by(TOWID) %>% 
          filter(SVSPP == 15) %>%
          mutate(EXPCATCHNUM = sum(EXPCATCHNUM), EXPCATCHWT = sum(EXPCATCHWT),
                 CATCHSEX = as.integer(3)) %>% 
          distinct()
          #summarise(EXPCATCHNUM = sum(EXPCATCHNUM), EXPCATCHWT = sum(EXPCATCHWT))
...
# adds observations of aggregated skates (does not remove the skate species at this point)
bts <- bts %>%
  #subset(!(SVSPP %in% c(22, 23, 24, 26))) %>% 
  bind_rows(sk_bts)
