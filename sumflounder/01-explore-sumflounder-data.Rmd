...
### Filter out 2017 fall survey and 2020 spring survey
```{r}
# fall 
sumflounder_complete |> 
  filter(!is.na(AVGDEPTH), SEASON == "FALL", YEAR != 2017) |> 
  group_by(EST_YEAR, SEASON) |>
  summarise(med_wt = median(EXPCATCHWT), 
            max_wt = max(EXPCATCHWT), 
            range_wt = max_wt - med_wt, 
...
sumflounder_complete |> 
  filter(!is.na(AVGDEPTH), SEASON == "FALL", YEAR != 2017) |> 
  summarise(x = quantile(EXPCATCHWT, 0.95), y = quantile(EXPCATCHWT, 0.975), z = quantile(EXPCATCHWT, 0.99)) 

# spring 
sumflounder_complete |> 
  filter(!is.na(AVGDEPTH), SEASON == "SPRING", YEAR != 2020) |> 
  group_by(EST_YEAR, SEASON) |>
  summarise(med_wt = median(EXPCATCHWT), 
            max_wt = max(EXPCATCHWT), 
            range_wt = max_wt - med_wt, 
...
sumflounder_complete |> 
  ...
  summarise(x = quantile(EXPCATCHWT, 0.95), y = quantile(EXPCATCHWT, 0.975), z = quantile(EXPCATCHWT, 0.99))
```
By removing the years, this does not effect the quantile distribution of biomass 

```{r}
sumflounder_filtered |> filter(SEASON == "FALL") |> ggplot() + aes(x = AVGDEPTH, y = EXPCATCHWT) + geom_point() + geom_smooth() + facet_wrap(~SEASON)

sumflounder_filtered |> filter(SEASON == "SPRING") |> ggplot() + aes(x = AVGDEPTH, y = EXPCATCHWT) + geom_point() + geom_smooth() + facet_wrap(~SEASON)
```
...
```{r}
strata_95 <- strata |>
  right_join(csum_dist95, by = "STRATUM") |> # join the filtered data to the strata shapefile; maintains shapefile structure 
...
ggplot() +
  ...
  geom_sf(data = strata_95, aes(fill = TYPE), alpha = 0.6)+ #fill the strata polygons based on the values in the csum_prop column, will be overwritten with scale_fill_manual()
...
ggplot(sumflounder_csum_fall) + 
  ...
  labs(x = "Stratum", y = "Cumulative Biomass (kg)", title = "Summer Flounder Fall Cumulative Distribution of Biomass", subtitle = "The incomplete 2017 survey has been removed from the calculation, and a survey was not performed in 2020") +
  theme_bw() +
...
ggplot(sumflounder_csum_spr) + 
  ...
  labs(x = "Stratum", y = "Cumulative Biomass (kg)", title = "Summer Flounder Spring Cumulative Distribution of Biomass", subtitle = "The incomplete 2020 survey has been removed from the calculation") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, size = 5))
