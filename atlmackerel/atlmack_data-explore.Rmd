...
stratmeans <- readRDS(here("data", "rds", "retro-analysis", "strat-mu_all.rds")) |> filter(SVSPP == 121, SEASON == "SPRING", EST_YEAR %in% full_years)
...
### Remove Outliers 
Abundance indices when you remove the observations of biomass greater than the 99th quantile 
...
Include the outlier year when 
```{r}
atlmack_incl.mu <- atlmack_stratmu |> 
  filter(TYPE == "With Wind Included") |> 
  select(!c(COMNAME, SCINAME, sdlog, lower, upper)) |>
  mutate(data_src = "All data")

atlmack_incl.mu_comp <- bind_rows(atlmack_incl.mu, atlmack_incl.strat_no.out)

atlmack_incl.mu_diff <- atlmack_incl.mu_comp |> 
  #mutate(stratmu = ifelse(stratmu==0, 1, stratmu)) |>
  filter(EST_YEAR %in% c(2014:2019)) |> #filter for recent 5 years, skipping 2020
  #mutate(log_mu = log(stratmu)) |>
  arrange(desc(stratmu)) |>
