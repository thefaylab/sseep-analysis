...
### Biomass catch rate summaries
```{r, echo=TRUE}
# find quick summary of biomass catch rates in each year 
...
### Catch rates by depth plots
#### Presence 
```{r depth histogram, echo=TRUE}
# save spring tows only 
am_spring <- atlmack_complete |> filter(SEASON == "SPRING", !is.na(AVGDEPTH))

# relationship of depth with biomass (presence only)
ggplot(am_spring |> filter(EXPCATCHWT > 0)) + 
  aes(x = AVGDEPTH, y = EXPCATCHWT)+
  geom_point() + 
  geom_smooth() +
  facet_wrap(~EST_YEAR, scales = "free_y") + 
  labs(x = "Depth (m)", y = "Biomass (kg)", subtitle = "Relationship between depth and biomass observed (presence only)")

# relationship of depth with biomass (presence only) - removing depth outliers
ggplot(am_spring |> filter(EXPCATCHWT > 0, AVGDEPTH <= 200)) + 
  aes(x = AVGDEPTH, y = EXPCATCHWT)+
  geom_point() + 
  geom_smooth() +
  facet_wrap(~EST_YEAR, scales = "free_y") + 
  labs(x = "Depth (m)", y = "Biomass (kg)", subtitle = "Relationship between depth and biomass observed (presence only) with observations\nat depths greater than 200m removed")

# number of observations at each depth
ggplot(am_spring) +
  geom_histogram(aes(x = AVGDEPTH), color = "white") + 
  labs(x = "Depth (m)", y = "Fequency", subtitle = "Number of observations at each depth")


ggplot(am_spring, aes(x = AVGDEPTH)) + 
  geom_histogram(aes(y = after_stat(count / max(count)*100)), color = "white") + 
  geom_hline(yintercept = 5, color = "red") + # catch at 5% 
  labs(x = "Depth (m)", y = "Proportion of catch", subtitle = "Proportion of tows at each depth")
...
ggplot(am_spring |> filter(EXPCATCHWT == 0)) + 
  aes(x = AVGDEPTH)+
  geom_histogram() +
  labs(x = "Depth (m)", y = "Frequency", subtitle = "Number of tows by depth that did not catch Atlantic mackerel")

# relationship of depth with biomass (presence/absence only)
ggplot(am_spring) + 
  aes(x = AVGDEPTH, y = EXPCATCHWT)+
  geom_point() + 
  geom_smooth() +
  facet_wrap(~EST_YEAR, scales = "free_y") + 
  labs(x = "Depth (m)", y = "Biomass (kg)", subtitle = "Relationship between depth and biomass observed (presence/absence)")

# relationship of depth with biomass (presence/absence only) - removing depth outliers
ggplot(am_spring |> filter(AVGDEPTH <= 200)) +
  aes(x = AVGDEPTH, y = EXPCATCHWT)+
  geom_point() +
  geom_smooth() +
  facet_wrap(~EST_YEAR, scales = "free_y") +
  labs(x = "Depth (m)", y = "Biomass (kg)", subtitle = "Relationship between depth and biomass observed (presence/absence) with observations\nat depths greater than 200m removed")
...
# summarise observations of depth
summary(am_spring$AVGDEPTH)

# create depth bin based on summary and add to dataframe
...
# calculate cumulative biomass by depth 
...
# set 95% and 99% thresholds based on cumulative biomass
lim95 <- max(depth_csum$csum_bio)*0.95
lim99 <- max(depth_csum$csum_bio)*0.99
```

```{r echo=TRUE}
# plot cumulative biomass by depth with threshold lines 
ggplot(depth_csum) + 
...
gt(depth_csum) |>
  fmt_number() |>
  cols_label(depth_bin = "Depth bin", 
             total_bio = "Total biomass", 
             csum_bio = "Cumulative biomass", 
             csum_prop = "Proportion of biomass") |>
  tab_header(title = md("Proportion of cumulative biomass by depth bin"), 
             subtitle = md("Bin value is the upper limit of the range of depths included")) |>
   cols_width(everything() ~ px(150)) |>
  tab_options(table.width = pct(40), 
              column_labels.font.weight = "bold")

# kable(depth_csum, align = "lcccc", caption = "Spring Atlantic mackerel cumulative distribution of biomass by depth bin. Bin value is the upper limit of the range of depths included.", format.args = list(big.mark = ","), booktabs = TRUE) |>
#   kable_styling(full_width = T, fixed_thead = T, font_size = 14) 

```

**Result**: 95% of cumulative biomass occurs below 170m in depth, while 99% of cumulative biomass occurs below 200m

#### Without incomplete survey years 
```{r}
am_spring1 <- am_spring |> filter(EST_YEAR != 2020) 
summary(am_spring1$AVGDEPTH)

am_spring1$depth_bin <- case_when(am_spring1$AVGDEPTH <= 24 ~ 24, 
                                 am_spring1$AVGDEPTH  > 24 & am_spring1$AVGDEPTH  <= 36 ~ 36,
                                 am_spring1$AVGDEPTH  > 36 & am_spring1$AVGDEPTH  <= 48 ~ 48,
                                 am_spring1$AVGDEPTH  > 48 & am_spring1$AVGDEPTH  <= 60 ~ 60,
                                 am_spring1$AVGDEPTH  > 60 & am_spring1$AVGDEPTH  <= 72 ~ 72,
                                 am_spring1$AVGDEPTH  > 72 & am_spring1$AVGDEPTH  <= 84 ~ 84,
                                 am_spring1$AVGDEPTH  > 84 & am_spring1$AVGDEPTH  <= 96 ~ 96,
                                 am_spring1$AVGDEPTH  > 96 & am_spring1$AVGDEPTH  <= 108 ~ 108,
                                 am_spring1$AVGDEPTH  > 108 & am_spring1$AVGDEPTH  <= 120 ~ 120,
                                 am_spring1$AVGDEPTH  > 120 & am_spring1$AVGDEPTH  <= 132 ~ 132,
                                 am_spring1$AVGDEPTH  > 132 & am_spring1$AVGDEPTH  <= 144 ~ 144,
                                 am_spring1$AVGDEPTH  > 144 & am_spring1$AVGDEPTH  <= 156 ~ 156,
                                 am_spring1$AVGDEPTH  > 156 & am_spring1$AVGDEPTH  <= 168 ~ 168,
                                 am_spring1$AVGDEPTH  > 168 & am_spring1$AVGDEPTH  <= 180 ~ 180,
                                 am_spring1$AVGDEPTH  > 180 & am_spring1$AVGDEPTH  <= 192 ~ 192,
                                 am_spring1$AVGDEPTH  > 192 & am_spring1$AVGDEPTH  <= 204 ~ 204,
                                 am_spring1$AVGDEPTH  > 204 & am_spring1$AVGDEPTH  <= 216 ~ 216,
                                 am_spring1$AVGDEPTH  > 216 & am_spring1$AVGDEPTH  <= 228 ~ 228,
                                 am_spring1$AVGDEPTH  > 228 & am_spring1$AVGDEPTH  <= 240 ~ 240,
                                 am_spring1$AVGDEPTH  > 240 & am_spring1$AVGDEPTH  <= 252 ~ 252,
                                 am_spring1$AVGDEPTH  > 252 & am_spring1$AVGDEPTH  <= 264 ~ 264,
                                 am_spring1$AVGDEPTH  > 264 & am_spring1$AVGDEPTH  <= 276 ~ 276,
                                 am_spring1$AVGDEPTH  > 276 & am_spring1$AVGDEPTH  <= 288 ~ 288,
                                 am_spring1$AVGDEPTH  > 288 & am_spring1$AVGDEPTH  <= 300 ~ 300,
                                 am_spring1$AVGDEPTH  > 300 & am_spring1$AVGDEPTH  <= 312 ~ 312,
                                 am_spring1$AVGDEPTH  > 312 & am_spring1$AVGDEPTH  <= 324 ~ 324,
                                 am_spring1$AVGDEPTH  > 324 & am_spring1$AVGDEPTH  <= 336 ~ 336,
                                 am_spring1$AVGDEPTH  > 336 & am_spring1$AVGDEPTH  <= 348 ~ 348,
                                 am_spring1$AVGDEPTH  > 348 & am_spring1$AVGDEPTH  <= 360 ~ 360,
                                 am_spring1$AVGDEPTH  > 360 & am_spring1$AVGDEPTH <= 372 ~ 372
                                 )

depth_csum1 <- am_spring1 |> 
     group_by(depth_bin) |> 
     summarise(total_bio = sum(EXPCATCHWT), .groups = "drop") |> 
     arrange(desc(total_bio)) |>
     mutate(csum_bio = cumsum(total_bio), 
...
lim95.1 <- max(depth_csum1$csum_bio)*0.95
lim99.1 <- max(depth_csum1$csum_bio)*0.99
```
...
ggplot(depth_csum1) + 
  geom_point(aes(x = fct_reorder(as.character(depth_bin), csum_bio), y = csum_bio)) + 
  ylim(0, NA) + 
  geom_hline(yintercept = lim95.1, color = "red") +
  geom_hline(yintercept = lim99.1, color = "blue") +
  labs(x = "Depth (m)", y = "Cumulative Biomass (kg)", title = "AtlMack Cumulative Distribution of Biomass") +
  theme_bw() + 
  theme(axis.text.x = element_text(size = 8, hjust = 0.75, angle = 45, margin = margin(t = 2, r = 10, b = 10, l = 5, unit = "pt")), 
        plot.margin = margin(t = 5, r = 15, b = 5, l = 5))
...
# kable(depth_csum1, align = "lcccc", caption = "Spring Atlantic mackerel cumulative distribution of biomass by depth bin. Bin value is the upper limit of the range of depths included.", format.args = list(big.mark = ","), booktabs = TRUE) |>
#   kable_styling(full_width = T, fixed_thead = T, font_size = 14) 
...
**Result:** Removing 2020 made little difference, but still should be removed for prosterity.
...
atlmack_stratmu <- bind_rows(atlmack_incl.strat$stratmu, atlmack_precl.strat$stratmu) |>
  group_by(EST_YEAR, effort) |> 
...
atlmack_stratmu_no.out <- bind_rows(atlmack_incl.strat_no.out$stratmu, atlmack_precl.strat_no.out$stratmu) |> 
  mutate(sdlog = sqrt(log(1+(sqrt(stratvar)/stratmu)^2)), #logistic standard deviation
         lower = qlnorm(0.025, log(stratmu), sdlog), # lower quantile of the logistic normal distribution
...
```{r}
atlmack_incl.mu <- bind_rows(atlmack_incl.strat$stratmu, atlmack_incl.strat_no.out$stratmu)
